...
Porém, como jó foi informado, a especialidade deles é coxinha, não matemática, então será que você pode ajudá-los? Com base no número total de coxinhas 
consumidas e o número de participantes na competição, o dono do restaurante precisa que você desenvolva um programa para saber a quantidade média de coxinha 
...
Entrada
A entrada consiste de uma única linha que contém dois inteiros H e P (1 ≤ H, P ≤ 1000) indicando respectivamente o número total de coxinhas consumidas e o 
...
namespace DesafioCoxinha
{
    class CoxinhaDoBueno    
    {
        static void Main(string[] args)
        {
            string[] line = Console.ReadLine().Split(" ");

            float H = int.Parse(line[0]);
            float P = int.Parse(line[1]);

            if (1 <= H && P <= 1000)
            {
                double mediaCoxinha = (H / P);

                if (H % P != 0)
                    Console.WriteLine(Math.Round(mediaCoxinha, 2).ToString("0.00"));
                else
                    Console.WriteLine(mediaCoxinha.ToString("0.00"));
            }
